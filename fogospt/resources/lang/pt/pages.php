...
return [
    'seo' => [
        ...
        'mapCustom'   => [ 'title' => 'Mapa Personalizado de Incêndios',        'description' => 'Mapa de incêndios em Portugal filtrado pelos concelhos, estados e camadas escolhidos.' ],
        'mapConfigurator' => [ 'title' => 'Configurar Mapa Partilhável',        'description' => 'Cria um mapa personalizado com URL fixo — escolhe concelhos, estados e camadas.' ],
    ...
    'information' => [
        ...
        'satellitePerimeter' => [
            ...
            'intro' => 'Na página de cada incidente aparece, no fundo, um mapa com dois produtos derivados de satélite fornecidos pelo <a href="https://foco.vost.pt" target="_blank" rel="noopener">FOCO</a> (Fire Observation and Characterisation Observatory, da VOST Portugal): o <strong>perímetro</strong> actual do fogo e uma <strong>simulação da propagação para as próximas 6 horas</strong>. O produto do satélite Meteosat Third Generation (MTG) é gerado a cada 10 minutos; o bloco só aparece quando existe dado satélite associado àquela ocorrência.',
            'perimeter' => [
                'title' => 'Perímetro (polígono vermelho)',
                'description' => 'Traça a área que os satélites Meteosat Third Generation (MTG) estão a ver com energia radiada suficiente. Cada píxel do satélite tem 2 a 4 km de largura sobre Portugal; o FOCO agrupa as detecções de FRP (Fire Radiative Power — potência radiativa do fogo, em megawatts) das últimas passagens e desenha um polígono. <strong>Não é</strong> a área ardida oficial do ICNF (compilada a posteriori); é uma indicação em tempo quase-real de onde o fogo está a libertar energia agora.',
            ],
            'simulation' => [
            ...
            'use' => [
                ...
                'description' => 'É informação de apoio à situação. O perímetro ajuda a perceber a dimensão real do que está a arder no momento; a simulação ajuda a antecipar o que fica na trajectória — aldeias, estradas, infraestruturas — nas próximas horas. O bloco vem também enriquecido com a lista de concelhos e freguesias afectados, calculada por intersecção espacial com a CAOP 2025 da DGT. O selo por baixo do título indica a hora da última passagem satélite processada.',
            ],
            'limits' => [
                'title' => 'Complementa mas não substitui — limitações importantes',
                'description' => 'O FOCO <strong>não é um detector de fogos nascentes</strong>: um incêndio com 1 ou 2 hectares raramente atinge o limiar de energia (50–100 MW) que o satélite precisa para o "ver". Só quando o fogo escapa ao ataque inicial e cresce é que costuma aparecer no MTG. Em resumo: <em>fogos.pt detecta o despacho</em> (o fogo pode ainda ser pequeno); <em>FOCO detecta a energia real irradiada</em> — são complementares. A simulação é um envelope heurístico, não uma previsão: não modela projecção de fagulhas (spotting), supressão activa nem piroconvecção; assume o vento previsto e, se rodar bruscamente ou passar uma frente, a evolução real pode divergir. Fogos maioritariamente do lado espanhol da fronteira aparecem como "Fora da cobertura da simulação", porque as camadas de combustível cobrem apenas o continente. Estes produtos <strong>nunca substituem</strong> a informação oficial da ANEPC, IPMA ou ICNF.',
            ],
            'source' => 'Dados fornecidos pelo <a href="https://foco.vost.pt" target="_blank" rel="noopener">FOCO — VOST Portugal</a>, a partir do produto MTG MTFRPPixel do LSA SAF (EUMETSAT, alojado no IPMA), PyroCast (VOST) para taxa de propagação e humidade do combustível, COS2023 (DGT) para cobertura do solo e carga de combustível, Copernicus GLO-90 DEM para o relevo, e CAOP 2025 (DGT) para os limites administrativos de concelhos e freguesias.',
        ],
        'ipmaCharts' => [
